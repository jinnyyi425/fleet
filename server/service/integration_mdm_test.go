...
func (s *integrationMDMTestSuite) TestProfileManagement() {
	...
	s.pushProvider.PushFunc = func(pushes []*mdm.Push) (map[string]*push.Response, error) {
		...
		require.Equal(t, pushes[0].PushMagic, "pushmagic"+mdmDevice.Serial)
...
func createHostThenEnrollMDM(ds fleet.Datastore, fleetServerURL string, t *testing.T) (*fleet.Host, *mdmtest.TestDevice) {
	desktopToken := uuid.New().String()
	mdmDevice := mdmtest.NewTestDevice(fleetServerURL, desktopToken, true)
	fleetHost, err := ds.NewHost(context.Background(), &fleet.Host{
		...
		UUID:           mdmDevice.UUID,
		HardwareSerial: mdmDevice.Serial,
	})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestDEPProfileAssignment() {
	...
	enrollmentProfile := fetchEnrollmentProfile(t, s.ds, s.server.URL)
	enrollInfo, err := mdmtest.ParseEnrollmentProfile(enrollmentProfile)
	require.NoError(t, err)

	// Enroll one of the hosts
	mdmDevice := mdmtest.NewDEPTestDevice(*enrollInfo, true)
	mdmDevice.Serial = devices[0].SerialNumber
	err = mdmDevice.Enroll()
	require.NoError(t, err)
...
func fetchEnrollmentProfile(t *testing.T, ds *mysql.Datastore, fleetServerURL string) []byte {
	var token string
	mysql.ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &token,
			`SELECT token FROM mdm_apple_enrollment_profiles`)
	})
	request, err := http.NewRequest("GET", fleetServerURL+apple_mdm.EnrollPath+"?token="+token, nil)
	require.NoError(t, err)
	response, err := fleethttp.NewClient().Do(request)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, response.StatusCode)
	defer response.Body.Close()
	mobileConfig, err := io.ReadAll(response.Body)
	require.NoError(t, err)
	return mobileConfig
}
...
func (s *integrationMDMTestSuite) TestAppleMDMDeviceEnrollment() {
	...
	mdmDeviceA := mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)
	err := mdmDeviceA.Enroll()
	require.NoError(t, err)
	mdmDeviceB := mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)
	...
	require.JSONEq(t, fmt.Sprintf(`{"host_serial": "%s", "host_display_name": "%s (%s)", "installed_from_dep": false}`, mdmDeviceA.Serial, mdmDeviceA.Model, mdmDeviceA.Serial), string(*details[len(details)-2]))
	require.JSONEq(t, fmt.Sprintf(`{"host_serial": "%s", "host_display_name": "%s (%s)", "installed_from_dep": false}`, mdmDeviceB.Serial, mdmDeviceB.Model, mdmDeviceB.Serial), string(*details[len(details)-1]))
	...
	for _, activity := range activities.Activities {
		if activity.Type == "mdm_unenrolled" {
			...
			require.Nil(t, activity.ActorFullName)
			details = append(details, activity.Details)
			require.JSONEq(t, fmt.Sprintf(`{"host_serial": "%s", "host_display_name": "%s (%s)", "installed_from_dep": false}`, mdmDeviceA.Serial, mdmDeviceA.Model, mdmDeviceA.Serial), string(*activity.Details))
		}
	}
	require.True(t, found)
}
...
func (s *integrationMDMTestSuite) TestDeviceMultipleAuthMessages() {
	...
	mdmDevice := mdmtest.NewDEPTestDevice(mdmtest.EnrollInfo{
		SCEPChallenge: s.fleetCfg.MDM.AppleSCEPChallenge,
		SCEPURL:       s.server.URL + apple_mdm.SCEPPath,
		MDMURL:        s.server.URL + apple_mdm.MDMPath,
	}, true)
...
func (s *integrationMDMTestSuite) TestBootstrapPackageStatus() {
	...
	noTeamDevices := []deviceWithResponse{
		{"Acknowledge", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Acknowledge", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Acknowledge", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Error", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Offline", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Offline", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Pending", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
		{"Pending", mdmtest.NewDEPTestDevice(mdmEnrollInfo, true)},
	}
